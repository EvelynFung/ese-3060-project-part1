"""
airbench94_cutmix.py
Modified with CutMix augmentation for improved regularization and training efficiency.
Reference: CutMix: Regularization Strategy to Train Strong Classifiers (Zhang et al., ICCV 2019)
"""
...
class CifarLoader:
    def __init__(self, path, train=True, batch_size=500, aug=None, drop_last=None, shuffle=None, gpu=0):
        ...
        for k in self.aug.keys():
            assert k in ['flip', 'translate', 'cutmix_alpha', 'cutmix_prob'], 'Unrecognized key: %s' % k
...
def main(run, cutmix_prob=0.5, cutmix_alpha=1.0):
    ...
    # MODIFIED: Pass CutMix parameters
    aug_config = hyp['aug'].copy()
    aug_config['cutmix_prob'] = cutmix_prob
    aug_config['cutmix_alpha'] = cutmix_alpha
    
    loss_fn = nn.CrossEntropyLoss(label_smoothing=hyp['opt']['label_smoothing'], reduction='none')
    test_loader = CifarLoader('cifar10', train=False, batch_size=2000)
    train_loader = CifarLoader('cifar10', train=True, batch_size=batch_size, aug=aug_config)
    ...
    for epoch in range(ceil(epochs)):
        ...
        for inputs, labels in train_loader:
            ...
            if torch.rand(1).item() < cutmix_prob:
                ...
                loss = lam * loss_fn(outputs, labels_a).sum() + (1 - lam) * loss_fn(outputs, labels_b).sum()
            else:
                outputs = model(inputs)
...
if __name__ == "__main__":
    ...
    print(f"\nTesting CutMix with prob={args.cutmix_prob}, alpha={args.cutmix_alpha}")
    print_columns(logging_columns_list, is_head=True)
    main('warmup', cutmix_prob=args.cutmix_prob, cutmix_alpha=args.cutmix_alpha)
    
    accs = []
    times = []
    for run in range(args.num_runs):
        acc, time = main(run, cutmix_prob=args.cutmix_prob, cutmix_alpha=args.cutmix_alpha)
        accs.append(acc)
        times.append(time)
    ...
    print(f'\nResults for cutmix_prob={args.cutmix_prob}, alpha={args.cutmix_alpha}:')
    print(f'Mean Accuracy: {accs.mean():.4f} ± {accs.std():.4f}')
    print(f'Average Time: {times_avg:.2f}s')
    ...
    log_dir = os.path.join('logs', f'cutmix_p{args.cutmix_prob}_a{args.cutmix_alpha}_{str(uuid.uuid4())[:8]}')
